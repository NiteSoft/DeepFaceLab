...
def tdict2kw_conv2d ( w, b ):
    if b is not None:
        return [ np.moveaxis(w.numpy(), [0,1,2,3], [3,2,0,1]), b.numpy() ]
    else:
        return [ np.moveaxis(w.numpy(), [0,1,2,3], [3,2,0,1])]
...
def main():
    ct_1_filepath = r'D:\DFLBuild\test\ct_00001.jpg'
    ct_1_img = cv2.imread(ct_1_filepath) / 255.0
    ct_1_img_shape = ct_1_img.shape    
    ct_1_dflimg = DFLJPG.load ( ct_1_filepath)
    
    ct_2_filepath = r'D:\DFLBuild\test\ct_00002.jpg'
    ct_2_img = cv2.imread(ct_2_filepath)/ 255.0
    ...
    model_weights = torch.load(r"D:\DFLBuild\test\RankSRGAN_NIQE.pth")
    ...
    RankSRGAN_model.save (r"D:\DFLBuild\test\RankSRGAN.h5") 
    
    RankSRGAN_model_func = K.function (RankSRGAN_model.inputs, RankSRGAN_model.outputs)   
    
    image = cv2.imread(r'D:\DFLBuild\test\00002.jpg').astype(np.float32) / 255.0
    ...
    img_src = cv2.imread(r'D:\DFLBuild\test\ct_src.jpg').astype(np.float32) / 255.0
    img_src = np.expand_dims (img_src, 0)
    img_src_shape = img_src.shape

    img_trg = cv2.imread(r'D:\DFLBuild\test\ct_trg.jpg').astype(np.float32) / 255.0
